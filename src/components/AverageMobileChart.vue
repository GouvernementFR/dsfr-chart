    ...
    <div class="widget_container fr-grid-row" :id="widgetId">
        <div class="r_col fr-col-12">
            <div class="chart">
                <div class="linechart_tooltip">
                    ...
                    <div class="tooltip_body">
                        <div class="tooltip_value">
                            <span class="tooltip_dot" v-bind:style="{ 'background-color': colormParse }"></span>
                        </div>
                    </div>
                </div>
                <canvas :id="chartId"></canvas>
                <div class="flex fr-mt-3v" :style="{ 'margin-left': isSmall ? '0px' : style }">
                    <span class="legende_dot" v-bind:style="{ 'background-color': colormParse }"></span>
                    <p class="fr-text--sm fr-text--bold fr-ml-1w fr-mb-0">{{ capitalize(namem) }}</p>
                </div>  
                <div class="flex fr-mt-3v" :style="{ 'margin-left': isSmall ? '0px' : style }">
                ...
                <div v-for="(item, index) in vlineParse" :key="item" class="flex fr-mt-3v fr-mb-1v"
                    :style="{ 'margin-left': isSmall ? '0px' : style }">
                    <span class="legende_dash_line1" v-bind:style="{ 'background-color': vlineColorParse[index] }"></span>
                    <span class="legende_dash_line2" v-bind:style="{ 'background-color': vlineColorParse[index] }"></span>
                    <p class="fr-text--sm fr-text--bold fr-ml-1w fr-mb-0">{{ capitalize(vlineNameParse[index]) }}</p>
                </div>
...
export default {
    ...
    data() {
        return {
            ...
            colormParse: undefined,
            colorPrecisionm: '#161616',
            colorHover: undefined,
            colormHover: undefined,
            isSmall: false
        }
    },
    props: {
    ...
    methods: {
        resetData() {
            ...
            this.colormParse = undefined
            this.colorPrecisionm = '#161616'
            this.colorHover = undefined
            this.colormHover = undefined
        },
        getData() {
            const self = this
            ...
            this.ymparse = JSON.parse(this.ym)
            ...
            // const backgroundColor = this.getHexaFromName(this.colorm)
            ...
            self.datasets = [
                {
                    ...
                    fill: false,
                    borderColor: this.colormParse,
                    hoverBorderColor: self.colormHover,
                    hoverBackgroundColor: self.colormHover,
                    mThickness: 32,
                    type: 'line',
                    mPercentage: 0.5,
                    yAxisID: 'yAxisL',
                    ...
                    afterDraw: function (chart, args, options) {
                        if (chart.tooltip._active !== undefined) {
                            if (chart.tooltip._active.length !== 0) {
                                ...
                                const yAxisR = chart.scales.yAxisL
                                const yAxisL = chart.scales.yAxisL
                                const xAxis = chart.scales['x-axis-0']

                                const y = yAxisL.getPixelForValue(self.yparse[index])
                                const ym = yAxisL.getPixelForValue(self.ymparse[index])
                                ...
                                ctx.lineWidth = '1'
                                ctx.strokeStyle = self.colorPrecisionm
                                ctx.setLineDash([10, 5])
                                ctx.stroke()
                                ctx.restore()
                    ...
                    tooltips: {
                        ...
                        custom: function (context) {
                            ...
                            if (tooltipModel.body) {
                                ...
                                const color = [self.colormParse, self.colorParse]
        ...
        loadColors() {
            ...
            this.colormParse = this.getHexaFromName(this.colorm)
            this.colorHover = this.getHexaFromName(this.color, { hover: true })
            this.colormHover = this.getHexaFromName(this.colorm, { hover: true })
        ...
        changeColors(theme) {
            ...
            if (theme === 'light') {
                ...
                this.colorPrecisionm = '#FFFFFF'
            }
            this.chart.data.datasets[0].backgroundColor = this.colormParse
            this.chart.data.datasets[0].borderColor = this.colormParse
            this.chart.data.datasets[1].borderColor = this.colorParse

            this.chart.data.datasets[0].hoverBackgroundColor = this.colormHover
            this.chart.data.datasets[0].hoverBorderColor = this.colormHover
